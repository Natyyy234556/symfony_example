...
class ProductoController extends AbstractController
{
  ...
  public function readAll(EntityManagerInterface $entityManager, Request $request): JsonResponse
  {
    ...
    public function readOne(EntityManagerInterface $entityManager,GeneradorDeMensaje $generadorDeMensajes, string $nombre): JsonResponse
    {
    ...
    public function update(EntityManagerInterface $entityManager, string $nombre,GeneradorDeMensaje $generadorDeMensajes, Request $request): JsonResponse
    {
        ...
        $entityManager->flush();
        return $this->json(['message'=>$generadorDeMensajes->getMensaje(0).'Se actualizaron los datos del producto.', 'data' => $data]);
    }

    #[Route('/{nombre}', name: 'app_usuario_delete', methods: ['DELETE'])]
    public function delete(EntityManagerInterface $entityManager, string $nombre, GeneradorDeMensaje $generadorDeMensajes, Request $request): JsonResponse
    {
        ...
        // Se aplican los cambios de la entidad en la bd
        $entityManager->flush();
        return $this->json(['message'=> $generadorDeMensajes->getMensaje(0).'Se elimino el producto.', 'data' =>$data]);
    }
}
